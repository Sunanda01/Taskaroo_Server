...
const userController = {
  async refreshAccessToken(req, res, next) {
    try {
      ...
      if (!incomingRefreshToken)
        return next(customErrorHandling.invalidToken("Invalid Token"));
  ...
  async login(req, res, next) {
    ...
    try {
      ...
      if (!user.verified)
        return next(
          customErrorHandling.userNotValid("User is not Verified!!!!")
        );
      const { accessToken, refreshToken } = await generateAccessAndRefreshToken(
        user._id
      );
  ...
  async updatePassword(req, res, next) {
    try {
      ...
      if (!validatePassword)
        return next(customErrorHandling.userNotValid("Invalid Password"));
      const update = {};
      const salt = await bcrypt.genSalt(Number(bcrypt_SaltLevel));
